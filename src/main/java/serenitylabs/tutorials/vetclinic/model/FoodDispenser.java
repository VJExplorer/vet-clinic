...
public class FoodDispenser {
    ...
    private static final Map<Breed,Integer> GRAMS_BY_KILO_BY_PET = new HashMap<>();
    static {
        BRAND_FOR_BREED.put(Breed.Dog,PetFood.FidosFood);
        BRAND_FOR_BREED.put(Breed.Cat,PetFood.KittyKat);
        GRAMS_BY_KILO_BY_PET.put(Breed.Dog,20);
        GRAMS_BY_KILO_BY_PET.put(Breed.Cat,10);
    }



    public Meal prepareMealFor(Pet pet) {
        return new Meal(getRightAmountOfFoodFor(pet),rightFoodFor(pet));
    }
    private PetFood rightFoodFor(Pet pet) {
        return BRAND_FOR_BREED.get(pet.getBreed());
    }

    private double getRightAmountOfFoodFor(Pet pet) {
        return pet.getWeightInKilos() * GRAMS_BY_KILO_BY_PET.get(pet.getBreed());
    }
